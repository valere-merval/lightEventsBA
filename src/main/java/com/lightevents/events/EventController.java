...
public class EventController {
    ...
    @GetMapping public List<Event> list(@RequestParam(defaultValue="true") boolean publishedOnly, @RequestParam(required=false) String country, @RequestParam(required=false) String city, @RequestParam(required=false) String category, @RequestParam(required=false) String organizer, @RequestParam(required=false) String from, @RequestParam(required=false) String to, @RequestParam(defaultValue="true") boolean upcomingOnly) { return publishedOnly ? service.published(country,city,category,organizer,from,to,upcomingOnly) : service.all(); }
    ...
    @PostMapping public Event create(@RequestHeader(value="X-LightEvents-Token", required=false) String token, @Valid @RequestBody EventDtos.CreateEventRequest request) { return service.create(request, accounts.requireOrganizer(token)); }
    @PostMapping("/{id}/tickets") public TicketType addTicket(@RequestHeader(value="X-LightEvents-Token", required=false) String token, @PathVariable Long id, @Valid @RequestBody EventDtos.CreateTicketRequest request) { Account organizer = accounts.requireOrganizer(token); return service.addTicket(id, request, organizer); }
