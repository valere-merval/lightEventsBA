...
public class NotificationService {
    ...
    public Map<String, Object> sendEmailWithAttachment(String to, String subject, String body, String filename, byte[] bytes) throws Exception {
        if (blank(to)) return Map.of("sent", false, "reason", "missing email");
        MimeMessage message = mailSender.createMimeMessage();
        MimeMessageHelper helper = new MimeMessageHelper(message, true, "UTF-8");
        helper.setFrom(from); helper.setTo(to); helper.setSubject(subject); helper.setText(body);
        helper.addAttachment(filename, new ByteArrayResource(bytes));
        mailSender.send(message);
        return Map.of("sent", true, "channel", "email", "to", to, "attachment", filename);
    }
